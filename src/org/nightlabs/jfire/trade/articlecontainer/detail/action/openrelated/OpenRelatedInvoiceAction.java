...
public class OpenRelatedInvoiceAction extends OpenRelatedAction {
	...
	protected boolean calculateEnabledWithArticles(Set<Article> articles) {
		setText(Messages.getString("org.nightlabs.jfire.trade.articlecontainer.detail.action.openrelated.OpenRelatedInvoiceAction.action.text.disabled")); //$NON-NLS-1$
		InvoiceID invoiceID = getCommonInvoiceID(articles);
		if (invoiceID != null) {
			setText(
					String.format(Messages.getString("org.nightlabs.jfire.trade.articlecontainer.detail.action.openrelated.OpenRelatedInvoiceAction.action.text.enabled"), //$NON-NLS-1$
