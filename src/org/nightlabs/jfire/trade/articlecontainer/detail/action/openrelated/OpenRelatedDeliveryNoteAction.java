...
public class OpenRelatedDeliveryNoteAction extends OpenRelatedAction {
	...
	protected boolean calculateEnabledWithArticles(Set<Article> articles) {
		setText(Messages.getString("org.nightlabs.jfire.trade.articlecontainer.detail.action.openrelated.OpenRelatedDeliveryNoteAction.action.text.disabled")); //$NON-NLS-1$
		DeliveryNoteID deliveryNoteID = getCommonDeliveryNoteID(articles);
		if (deliveryNoteID != null) {
			setText(
					String.format(Messages.getString("org.nightlabs.jfire.trade.articlecontainer.detail.action.openrelated.OpenRelatedDeliveryNoteAction.action.text.enabled"), //$NON-NLS-1$
