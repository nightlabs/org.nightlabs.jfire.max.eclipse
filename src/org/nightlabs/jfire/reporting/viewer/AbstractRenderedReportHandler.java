...
//		}
	 ...
	 * 
	 * 
	 * @see org.nightlabs.jfire.reporting.viewer.RenderedReportHandler#prepareRenderedReportLayout(IProgressMonitor, RenderedReportLayout)
	 */
	public PreparedRenderedReportLayout prepareRenderedReportLayout(IProgressMonitor monitor, RenderedReportLayout layout) {
		...
		try {
			RCPUtil.getSaveProgressMonitor(monitor).setTaskName(Messages.getString("org.nightlabs.jfire.reporting.viewer.AbstractRenderedReportHandler.saveProgressMonitor.storeLayoutToDiskTask.name")); //$NON-NLS-1$
			if (!zip.exists()) {
//				if (!zip.delete())
...
//				}
		...
		if (layout.getHeader().isZipped()) {
			RCPUtil.getSaveProgressMonitor(monitor).setTaskName(Messages.getString("org.nightlabs.jfire.reporting.viewer.AbstractRenderedReportHandler.saveProgressMonitor.unzipLayoutTask.name")); //$NON-NLS-1$
			try {
