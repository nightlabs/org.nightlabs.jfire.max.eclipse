...
extends WizardHopPage
{
	...
	{
		super(PaymentSummaryPage.class.getName(), Messages.getString("org.nightlabs.jfire.trade.transfer.wizard.PaymentSummaryPage.title")); //$NON-NLS-1$
	}
	...
	public void onShow()
	{
		...
		for (PaymentEntryPage paymentEntryPage : paymentEntryPages) {
			if (Payment.PAYMENT_DIRECTION_INCOMING.equals(paymentEntryPage.getPayment().getPaymentDirection()))
				sb.append(Util.addTrailingChars(Messages.getString("org.nightlabs.jfire.trade.transfer.wizard.PaymentSummaryPage.paymentDirection_receive"), 15, ' ')); //$NON-NLS-1$
			else if (Payment.PAYMENT_DIRECTION_OUTGOING.equals(paymentEntryPage.getPayment().getPaymentDirection()))
				sb.append(Util.addTrailingChars(Messages.getString("org.nightlabs.jfire.trade.transfer.wizard.PaymentSummaryPage.paymentDirection_pay"), 15, ' ')); //$NON-NLS-1$
			else
