...
public class ValueProviderGUITimePeriod extends AbstractValueProviderGUI<TimePeriod> {
	...
	public Control createGUI(Composite parent) {
		if (wrapper == null) {
			...
			fromEdit = new DateTimeEdit(group, DateFormatter.FLAGS_DATE_LONG_TIME_HM, Messages.getString("org.nightlabs.jfire.reporting.parameter.guifactory.simpletypes.ValueProviderGUITimePeriod.fromDateTimeEdit.caption")); //$NON-NLS-1$
			toEdit = new DateTimeEdit(group, DateFormatter.FLAGS_DATE_LONG_TIME_HM, Messages.getString("org.nightlabs.jfire.reporting.parameter.guifactory.simpletypes.ValueProviderGUITimePeriod.toDateTimeEdit.caption")); //$NON-NLS-1$
		}
		return wrapper;
	}
