...
extends Dialog
{
	...
	protected Control createDialogArea(Composite parent)
	{
		...
		new Label(area, SWT.WRAP).setText(Messages.getString("org.nightlabs.jfire.trade.admin.gridpriceconfig.StorePriceConfigsConfirmationDialog.description")); //$NON-NLS-1$
		...
		ProductType dummy = new ProductType("dummy", "dummy", null, new LegalEntity("", "", ""), ProductType.INHERITANCE_NATURE_LEAF, ProductType.PACKAGE_NATURE_OUTER) { //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
			...
			public I18nText getName()
			{
				if (name == null) {
					...
					name.setText(Locale.getDefault().getLanguage(), Messages.getString("org.nightlabs.jfire.trade.admin.gridpriceconfig.StorePriceConfigsConfirmationDialog.pseudoEntry_loading")); //$NON-NLS-1$
				}
				return name;
			}
			@Implement
		...
		Job job = new Job(Messages.getString("org.nightlabs.jfire.trade.admin.gridpriceconfig.StorePriceConfigsConfirmationDialog.loadAffectedProductTypesJob.name")) { //$NON-NLS-1$
