package org.nightlabs.jfire.simpletrade.admin.producttype;
...
//	private Button editNestedProductType;
	...
	public ProductTypeDetailComposite(Composite parent, int style, IDirtyStateManager dirtyStateManager)
	{
		...
		nameLabel.setText(Messages.getString("org.nightlabs.jfire.simpletrade.admin.producttype.ProductTypeDetailComposite.nameLabel.text")); //$NON-NLS-1$
...
//		inheritProductTypeName.setText("inherit");
			...
			public void widgetSelected(SelectionEvent e)
			{
				if (inheritProductTypeName.getSelection()) {
					if (productType != null) {
						...
						Job job = new Job(Messages.getString("org.nightlabs.jfire.simpletrade.admin.producttype.ProductTypeDetailComposite.loadExtendedProductTypeJob.name")) { //$NON-NLS-1$
							@Override
		...
		new Label(compNestHeader, SWT.NONE).setText(Messages.getString("org.nightlabs.jfire.simpletrade.admin.producttype.ProductTypeDetailComposite.nestedProductTypesLabel.text")); //$NON-NLS-1$
...
//		editNestedProductType.setText("Edit");
	...
	{		
		...
		if (inherited) {
			Job job = new Job(Messages.getString("org.nightlabs.jfire.simpletrade.admin.producttype.ProductTypeDetailComposite.inheritNestedProductTypesJob.name")) { //$NON-NLS-1$
				@Implement
				protected IStatus run(ProgressMonitor monitor) throws Exception
				{
					try {
...
//			new NotificationEvent(this, JDOHelper.getObjectId(spt)));
	...
	{
		Job loadJob = new Job(Messages.getString("org.nightlabs.jfire.simpletrade.admin.producttype.ProductTypeDetailComposite.loadProductTypeJob.name")) { //$NON-NLS-1$
			@Override
			protected IStatus run(ProgressMonitor monitor) throws Exception {
				Display.getDefault().asyncExec(new Runnable() {
					public void run() {
