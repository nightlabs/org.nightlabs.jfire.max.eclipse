...
import org.nightlabs.jfire.trade.account.editor.AccountEditor;
import org.nightlabs.jfire.trade.account.editor.AccountEditorInput;
...
extends AbstractTableComposite<Account> 
{
	...
	protected void createTableColumns(TableViewer tableViewer, Table table) 
	{
		...
		tableViewerColumn.getColumn().setText(Messages.getString("org.nightlabs.jfire.trade.overview.account.AccountListComposite.id"));  //$NON-NLS-1$
		...
		tableViewerColumn.getColumn().setText(Messages.getString("org.nightlabs.jfire.trade.overview.account.AccountListComposite.accountName"));	//$NON-NLS-1$
		tableViewerColumn.setLabelProvider(new ColumnLabelProvider() {
			@Override
			public String getText(Object element) {
		...
		tableViewerColumn.getColumn().setText(Messages.getString("org.nightlabs.jfire.trade.overview.account.AccountListComposite.owner"));	//$NON-NLS-1$
		tableViewerColumn.setLabelProvider(new ColumnLabelProvider() {
			@Override
			public String getText(Object element) {
				if (!(element instanceof Account)) 
		...
		tableViewerColumn.getColumn().setText(Messages.getString("org.nightlabs.jfire.trade.overview.account.AccountListComposite.anchorType"));	//$NON-NLS-1$
		tableViewerColumn.setLabelProvider(new ColumnLabelProvider() {
			@Override
			public String getText(Object element) {
				if (!(element instanceof Account)) 
		...
		tableViewerColumn.getColumn().setText(Messages.getString("org.nightlabs.jfire.trade.overview.account.AccountListComposite.balance"));	//$NON-NLS-1$
		tableViewerColumn.setLabelProvider(new ColumnLabelProvider() {
			@Override
			public String getText(Object element) {
				if (!(element instanceof Account)) 
			...
			for (Iterator<?> it = notificationEvent.getSubjects().iterator(); it.hasNext(); ) {
				...
				for(Account a : accountList){
					...
					if (anchorID.equals(dirtyObjectID.getObjectID())) {
						Job job = new Job(Messages.getString("org.nightlabs.jfire.trade.account.editor.AccountEditor.loadingAccountJob.name")) //$NON-NLS-1$
						{
							@Override
