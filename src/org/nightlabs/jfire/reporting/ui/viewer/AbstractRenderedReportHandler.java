...
//		}
	...
	public PreparedRenderedReportLayout prepareRenderedReportLayout(IProgressMonitor monitor, RenderedReportLayout layout) {
		...
		try {
			RCPUtil.getSaveProgressMonitor(monitor).setTaskName(Messages.getString("org.nightlabs.jfire.reporting.ui.viewer.AbstractRenderedReportHandler.saveProgressMonitor.storeLayoutToDiskTask.name")); //$NON-NLS-1$
			if (!zip.exists()) {
//				if (!zip.delete())
...
//				}
		...
		if (layout.getHeader().isZipped()) {
			RCPUtil.getSaveProgressMonitor(monitor).setTaskName(Messages.getString("org.nightlabs.jfire.reporting.ui.viewer.AbstractRenderedReportHandler.saveProgressMonitor.unzipLayoutTask.name")); //$NON-NLS-1$
			try {
