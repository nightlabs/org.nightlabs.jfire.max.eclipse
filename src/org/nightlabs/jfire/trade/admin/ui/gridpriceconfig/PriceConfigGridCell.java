...
public class PriceConfigGridCell
{
	...
	public void setFormula(String formula)
	{
		...
		if (oldFormula != formula) // workaround for bug in firePropertyChange(..) if both are null
			propertyChangeSupport.firePropertyChange(PROPERTY_FORMULA, oldFormula, formula); //$NON-NLS-1$
	}
	...
	public void setFallbackFormula(String formula)
	{
		...
		if (oldFormula != formula) // workaround for bug in firePropertyChange(..) if both are null
			propertyChangeSupport.firePropertyChange(PROPERTY_FALLBACK_FORMULA, oldFormula, formula); //$NON-NLS-1$
	}
