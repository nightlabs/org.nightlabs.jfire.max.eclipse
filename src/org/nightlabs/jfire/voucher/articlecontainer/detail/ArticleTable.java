...
implements ISelectionProvider
{
	...
	private static String getValidity(VoucherKey voucherKey)
	{
		return
				...
				(VoucherKey.VALIDITY_REDEEMED_COMPLETELY != voucherKey.getValidity() && voucherKey.isRedemptionExisting() ? (' '+Messages.getString("org.nightlabs.jfire.voucher.articlecontainer.detail.ArticleTable.validitySuffix_partiallyRedeemed")) : ""); //$NON-NLS-1$ //$NON-NLS-2$
	}
...
//			}
	...
	{		
		...
		col.setText(Messages.getString("org.nightlabs.jfire.voucher.articlecontainer.detail.ArticleTable.nameTableColumn.text")); //$NON-NLS-1$
//		col.setToolTipText("Name of the voucher");

		col = new TableColumn(table, SWT.LEFT);
		col.setText(Messages.getString("org.nightlabs.jfire.voucher.articlecontainer.detail.ArticleTable.keyTableColumn.text")); //$NON-NLS-1$
//		col.setToolTipText("Key (after delivery)");

		col = new TableColumn(table, SWT.LEFT);
		col.setText(Messages.getString("org.nightlabs.jfire.voucher.articlecontainer.detail.ArticleTable.validityTableColumn.text")); //$NON-NLS-1$
//		col.setToolTipText("Validity");

		col = new TableColumn(table, SWT.LEFT);
		col.setText(Messages.getString("org.nightlabs.jfire.voucher.articlecontainer.detail.ArticleTable.allocationStatusTableColumn.text"));		 //$NON-NLS-1$
//		col.setToolTipText("Status of the allocation");
		
		//////////// BEGIN Order, Offer, Invoice, DeliveryNote //////////
		if (!articleEdit.isInOrder() && !articleEdit.isInOffer()) {
			col = new TableColumn(table, SWT.LEFT);
			col.setText(Messages.getString("org.nightlabs.jfire.voucher.articlecontainer.detail.ArticleTable.orderTableColumn.text"));			 //$NON-NLS-1$
//			col.setToolTipText("Order");
		}

		if (!articleEdit.isInOffer()) {
			col = new TableColumn(table, SWT.LEFT);
			col.setText(Messages.getString("org.nightlabs.jfire.voucher.articlecontainer.detail.ArticleTable.offerTableColumn.text"));			 //$NON-NLS-1$
//			col.setToolTipText("Offer");
		}

		if (!articleEdit.isInInvoice()) {
			col = new TableColumn(table, SWT.LEFT);
			col.setText(Messages.getString("org.nightlabs.jfire.voucher.articlecontainer.detail.ArticleTable.invoiceTableColumn.text"));			 //$NON-NLS-1$
//			col.setToolTipText("Invoice");			
		}

		if (!articleEdit.isInDeliveryNote()) {
			col = new TableColumn(table, SWT.LEFT);
			col.setText(Messages.getString("org.nightlabs.jfire.voucher.articlecontainer.detail.ArticleTable.deliveryNoteTableColumn.text")); //$NON-NLS-1$
//			col.setToolTipText("Delivery Note");
		...
		columnPrice.setText(Messages.getString("org.nightlabs.jfire.voucher.articlecontainer.detail.ArticleTable.priceTableColumn.text"));		 //$NON-NLS-1$
