...
//		setHeaderText("Invoice " + invoice.getInvoiceID());
	...
	private NotificationListener invoiceChangedListener = new NotificationAdapterJob(Messages.getString("org.nightlabs.jfire.trade.articlecontainer.detail.invoice.InvoiceHeaderComposite.loadInvoicesJob.name")) { //$NON-NLS-1$
		public void notify(NotificationEvent notificationEvent)
		{
			ProgressMonitor monitor = getProgressMonitorWrapper();
			monitor.beginTask(Messages.getString("org.nightlabs.jfire.trade.articlecontainer.detail.invoice.InvoiceHeaderComposite.loadInvoicesMonitor.task.name"), 3); //$NON-NLS-1$
	...
	private void signalNextTransition(final SignalEvent event)
	{
		...
		Job job = new Job(Messages.getString("org.nightlabs.jfire.trade.articlecontainer.detail.invoice.InvoiceHeaderComposite.performTransitionJob.name")) { //$NON-NLS-1$
			@Implement
