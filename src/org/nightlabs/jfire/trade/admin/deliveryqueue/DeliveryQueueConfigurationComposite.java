package org.nightlabs.jfire.trade.admin.deliveryqueue;
...
public class DeliveryQueueConfigurationComposite extends XComposite {
	...
	public DeliveryQueueConfigurationComposite(Composite parent, IDirtyStateManager dirtyStateManager) {
		...
		new Label(spaceWrapper, SWT.NONE).setText(Messages.getString("org.nightlabs.jfire.trade.admin.deliveryqueue.DeliveryQueueConfigurationComposite.visibleDeliveryQueuesLabel.text")); //$NON-NLS-1$
		...
		addQueueButton.setText(Messages.getString("org.nightlabs.jfire.trade.admin.deliveryqueue.DeliveryQueueConfigurationComposite.addQueueButton.text")); //$NON-NLS-1$
		...
		editQueueButton.setText(Messages.getString("org.nightlabs.jfire.trade.admin.deliveryqueue.DeliveryQueueConfigurationComposite.editQueueButton.text")); //$NON-NLS-1$
		...
		delQueueButton.setText(Messages.getString("org.nightlabs.jfire.trade.admin.deliveryqueue.DeliveryQueueConfigurationComposite.removeQueueButton.text")); //$NON-NLS-1$
	...
	void populateDeliveryQueueList() {
		String loadingMsg = Messages.getString("org.nightlabs.jfire.trade.admin.deliveryqueue.DeliveryQueueConfigurationComposite.loadDeliveryQueuesJob.name"); //$NON-NLS-1$
...
//			pqTableComposite.getTableViewer().add(deliveryQueue);
	...
	void deleteSelectedDeliveryQueue() {
		if (MessageDialog.openQuestion(RCPUtil.getActiveWorkbenchShell(), Messages.getString("org.nightlabs.jfire.trade.admin.deliveryqueue.DeliveryQueueConfigurationComposite.deleteConfirmationDialog.title"), Messages.getString("org.nightlabs.jfire.trade.admin.deliveryqueue.DeliveryQueueConfigurationComposite.deleteConfirmationDialog.message")) == true) { //$NON-NLS-1$ //$NON-NLS-2$
			...
			if (JDOHelper.isDetached(selectedQueue)) {
				if (selectedQueue.hasPendingDeliveries()) {
					String message = Messages.getString("org.nightlabs.jfire.trade.admin.deliveryqueue.DeliveryQueueConfigurationComposite.DeliveryQueueDeletionImpossible.message"); //$NON-NLS-1$
					MessageDialog.openError(RCPUtil.getActiveWorkbenchShell(), Messages.getString("org.nightlabs.jfire.trade.admin.deliveryqueue.DeliveryQueueConfigurationComposite.DeliveryQueueDeletionImpossible.title"), message); //$NON-NLS-1$
					return;
				}
