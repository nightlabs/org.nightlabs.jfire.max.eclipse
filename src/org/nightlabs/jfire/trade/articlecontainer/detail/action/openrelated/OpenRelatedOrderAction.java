...
public class OpenRelatedOrderAction extends OpenRelatedAction {
	...
	protected boolean calculateEnabledWithArticles(Set<Article> articles) {
		setText(Messages.getString("org.nightlabs.jfire.trade.articlecontainer.detail.action.openrelated.OpenRelatedOrderAction.action.text.disabled")); //$NON-NLS-1$
		OrderID orderID = getCommonOrderID(articles);
		if (orderID != null) {
			setText(
					String.format(Messages.getString("org.nightlabs.jfire.trade.articlecontainer.detail.action.openrelated.OpenRelatedOrderAction.action.text.enabled"), //$NON-NLS-1$
					orderID.orderIDPrefix, ObjectIDUtil.longObjectIDFieldToString(orderID.orderID)
				)
			);
		}
		return orderID != null && !(getActiveGeneralEditorInput() instanceof GeneralEditorInputOrder);
	}
