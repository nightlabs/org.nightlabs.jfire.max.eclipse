...
package org.nightlabs.jfire.trade.admin.account;
...
extends DynamicPathWizardPage 
{
	...
	public CreateAccountEntryWizardPage(ProductType productTypeForAccount) {
		super(
				...
				Messages.getString("org.nightlabs.jfire.trade.admin.account.CreateAccountEntryWizardPage.title"), //$NON-NLS-1$
	...
	public Control createPageContents(Composite parent) {
		...
		getContainer().getShell().setText(Messages.getString("org.nightlabs.jfire.trade.admin.account.CreateAccountEntryWizardPage.windowTitle")); //$NON-NLS-1$
		...
		comboOwner = new XComboComposite<String>(wrapper, XComboComposite.getDefaultWidgetStyle(wrapper), 
			Messages.getString("org.nightlabs.jfire.trade.admin.account.CreateAccountEntryWizardPage.ownerCombo.caption")); //$NON-NLS-1$
		...
		comboCurrency = new XComboComposite<Currency>(wrapper, XComboComposite.getDefaultWidgetStyle(wrapper),	
				Messages.getString("org.nightlabs.jfire.trade.admin.account.CreateAccountEntryWizardPage.currencyCombo.caption"), new CurrencyLabelProvider()); //$NON-NLS-1$
...
//		getCombo().add(currency.getCurrencyID()+" ("+currency.getCurrencySymbol()+")"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		normalAccountRadio.setText(Messages.getString("org.nightlabs.jfire.trade.admin.account.CreateAccountEntryWizardPage.radioNormalAccount.text")); //$NON-NLS-1$
		...
		shadowAccountRadio.setText(Messages.getString("org.nightlabs.jfire.trade.admin.account.CreateAccountEntryWizardPage.radioSummaryAccount.text")); //$NON-NLS-1$
	...
	protected void updatePage() {
		if ( "".equals(accountNameEditor.getEditText()) ) { //$NON-NLS-1$
			updateStatus(Messages.getString("org.nightlabs.jfire.trade.admin.account.CreateAccountEntryWizardPage.errorMessageNoAnchorID")); //$NON-NLS-1$
			return;
		}
		if (comboCurrency.getSelectionIndex() < 0) {
			updateStatus(Messages.getString("org.nightlabs.jfire.trade.admin.account.CreateAccountEntryWizardPage.errorMessageNoCurrency")); //$NON-NLS-1$
			return;
		}
		updateStatus(null);
	}
	...
	protected String getDefaultPageMessage() {
		return Messages.getString("org.nightlabs.jfire.trade.admin.account.CreateAccountEntryWizardPage.defaultPageMessage"); //$NON-NLS-1$
	}
