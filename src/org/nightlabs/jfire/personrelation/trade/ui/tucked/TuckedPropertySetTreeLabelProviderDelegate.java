 ...
 */
public class TuckedPropertySetTreeLabelProviderDelegate extends AbstractPersonRelationTreeLabelProviderDelegate {
	@Override
	...
	public Image getJDOObjectImage(ObjectID jdoObjectID, Object jdoObject, int spanColIndex) {
		...
		TuckedPersonRelationTreeNode rootNode = treeNodeList.get(0); // We can simply pick the first because we are examining only the children -- and all children of a given node is the same!
			...
			if (childObjectID instanceof PersonRelationID) {
				...
				if (relationTypeToRoot == null)
					...
					if (!relationTypeToRoot.equals(personRelationType)) {
						...
						Image nodeIcon = SharedImages.getSharedImage(Activator.getDefault(), TuckedPersonRelationTreeLabelProviderDelegate.class, "");
		...
		Image nodeIcon = SharedImages.getSharedImage(Activator.getDefault(), TuckedPersonRelationTreeLabelProviderDelegate.class, personRelationTypeID);
		objectID2Image.put((PropertySetID) jdoObjectID, nodeIcon);
	 ...
	 */
	public TuckedPropertySetTreeLabelProviderDelegate(PersonRelationTreeController<TuckedPersonRelationTreeNode> personRelationTreeController) {
