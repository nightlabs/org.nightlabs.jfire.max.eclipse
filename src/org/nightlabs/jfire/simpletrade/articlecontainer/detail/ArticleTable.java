...
//					return SharedImages.getSharedImage(SimpletradePlugin.getDefault(), ArticleTable.class, "allocationPending");
//				else if (article.isReleaseAbandoned())
//					return SharedImages.getSharedImage(SimpletradePlugin.getDefault(), ArticleTable.class, "releaseAbandoned");
//				else if (article.isReleasePending())
//					return SharedImages.getSharedImage(SimpletradePlugin.getDefault(), ArticleTable.class, "releasePending");
//				else if (article.isAllocated())
//					return SharedImages.getSharedImage(SimpletradePlugin.getDefault(), ArticleTable.class, "allocated");
//				else
//					return SharedImages.getSharedImage(SimpletradePlugin.getDefault(), ArticleTable.class, "notAllocated");
		...
		public String getColumnText(Object element, int columnIndex)
		{
			...
			Article article = (Article) element;
			if (! (article.getProductType() instanceof SimpleProductType))
				return "article.productType is an instance of " + (article.getProductType() == null ? "null" : article.getProductType().getClass().getName()) + ", but must be a SimpleProductType!"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

			SimpleProductType simpleProductType = (SimpleProductType) article.getProductType();
	...
	{		
		...
		col.setText(Messages.getString("org.nightlabs.jfire.simpletrade.articlecontainer.detail.ArticleTable.nameTableColumn.text")); //$NON-NLS-1$
//		col.setToolTipText("Name");
		
		col = new TableColumn(table, SWT.LEFT);
		col.setText(Messages.getString("org.nightlabs.jfire.simpletrade.articlecontainer.detail.ArticleTable.allocationStatusTableColumn.text"));		 //$NON-NLS-1$
//		col.setToolTipText("Allocation Status");
		
		//////////// BEGIN Order, Offer, Invoice, DeliveryNote //////////
		if (!articleEdit.isInOrder() && !articleEdit.isInOffer()) {
			col = new TableColumn(table, SWT.LEFT);
			col.setText(Messages.getString("org.nightlabs.jfire.simpletrade.articlecontainer.detail.ArticleTable.orderTableColumn.text"));			 //$NON-NLS-1$
//			col.setToolTipText("Order");
		}

		if (!articleEdit.isInOffer()) {
			col = new TableColumn(table, SWT.LEFT);
			col.setText(Messages.getString("org.nightlabs.jfire.simpletrade.articlecontainer.detail.ArticleTable.offerTableColumn.text"));			 //$NON-NLS-1$
//			col.setToolTipText("Offer");
		}

		if (!articleEdit.isInInvoice()) {
			col = new TableColumn(table, SWT.LEFT);
			col.setText(Messages.getString("org.nightlabs.jfire.simpletrade.articlecontainer.detail.ArticleTable.invoiceTableColumn.text"));			 //$NON-NLS-1$
//			col.setToolTipText("Invoice");			
		}

		if (!articleEdit.isInDeliveryNote()) {
			col = new TableColumn(table, SWT.LEFT);
			col.setText(Messages.getString("org.nightlabs.jfire.simpletrade.articlecontainer.detail.ArticleTable.deliveryNoteTableColumn.text")); //$NON-NLS-1$
//			col.setToolTipText("DeliveryNote");
		...
		columnPrice.setText(Messages.getString("org.nightlabs.jfire.simpletrade.articlecontainer.detail.ArticleTable.priceTableColumn.text"));		 //$NON-NLS-1$
