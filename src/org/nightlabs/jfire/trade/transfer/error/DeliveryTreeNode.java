		...
		extends TransferTreeNode
{
	...
	public String getColumnText(int columnIndex)
	{
		switch (columnIndex) {
			case 0:
				return Messages.getString("org.nightlabs.jfire.trade.transfer.error.DeliveryTreeNode.delivery"); //$NON-NLS-1$
			case 1:
				return String.format(Messages.getString("org.nightlabs.jfire.trade.transfer.error.DeliveryTreeNode.articleCount"), new Integer(delivery.getArticleIDs().size())); //$NON-NLS-1$
			case 2:
				return delivery.isFailed() ? Messages.getString("org.nightlabs.jfire.trade.transfer.error.DeliveryTreeNode.failure") : Messages.getString("org.nightlabs.jfire.trade.transfer.error.DeliveryTreeNode.success"); //$NON-NLS-1$ //$NON-NLS-2$
			case 3:
				return delivery.isRolledBack() ? Messages.getString("org.nightlabs.jfire.trade.transfer.error.DeliveryTreeNode.rolledBack") : Messages.getString("org.nightlabs.jfire.trade.transfer.error.DeliveryTreeNode.notRolledBack"); //$NON-NLS-1$ //$NON-NLS-2$
			default:
				return ""; //$NON-NLS-1$
		}
	}
