 ...
 */
public class VoucherTypeTable extends AbstractTableComposite {
	...
	private static class ContentProvider implements IStructuredContentProvider {
		public Object[] getElements(Object inputElement) {
			if (inputElement instanceof Collection) {
				...
				throw new IllegalArgumentException("VoucherTypeTable.ContentProvider expects a Collection as inputElement. Recieved "+inputElement.getClass().getName()); //$NON-NLS-1$
		}
	...
	private static class LabelProvider extends org.eclipse.jface.viewers.LabelProvider implements ITableLabelProvider {
		...
		public String getColumnText(Object element, int columnIndex) {
			return ((VoucherType)element).getName().getText(Locale.getDefault().getLanguage());
		}
	}
	...
	public VoucherTypeTable(Composite parent, VoucherTypeQuickListFilter filter, int viewerStyle) {
		...
		getTableViewer().addSelectionChangedListener(new ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent event)
			{
				...
				VoucherTypeTable.this.filter.setSelectedProductTypeID(productTypeID);
			}
		});
	}
	...
	protected void createTableColumns(TableViewer tableViewer, Table table) {
		new TableColumn(table, SWT.LEFT).setText(Messages.getString("org.nightlabs.jfire.voucher.quicklist.VoucherTypeTable.voucherTypeNameTableColumn.text")); //$NON-NLS-1$
