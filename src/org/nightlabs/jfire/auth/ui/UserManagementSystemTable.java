...
/**
 * Table for representing {@link UserManagementSystem}s showing their names, types and leading status.
 * 
 * @author Denis Dudnik <deniska.dudnik[at]gmail{dot}com>
 *
 */
public class UserManagementSystemTable extends ActiveJDOObjectTableComposite<UserManagementSystemID, UserManagementSystem>{
	...
	private Set<UserManagementSystemID> addedUserManagementSystemsIDs;
	...
	private class UserManagementSystemController extends ActiveJDOObjectController<UserManagementSystemID, UserManagementSystem> {
		...
		protected Collection<UserManagementSystem> retrieveJDOObjects(ProgressMonitor monitor) {
			if (addedUserManagementSystemsIDs == null){
				...
				return UserManagementSystemDAO.sharedInstance().getUserManagementSystems(
						addedUserManagementSystemsIDs, USER_MANAGEMENT_SYSTEM_FETCH_GROUPS, NLJDOHelper.MAX_FETCH_DEPTH_NO_LIMIT, monitor);
			}
		}
	...
	/**
	 * Set {@link UserManagementSystemID}s to make this table display only needed {@link UserManagementSystem}s
	 * in case userManagementSystemIDs is not <code>null</code>, otherwise all objects will be displayed.
	 * Note that you need to call {@link #load()} or {@link #reload()} manually in order to update table items.
	 * 
	 * @param userManagementSystemIDs
	 */
	public void setUserManagementSystemsIDs(Set<UserManagementSystemID> userManagementSystemIDs){
		this.addedUserManagementSystemsIDs = userManagementSystemIDs;
	}
