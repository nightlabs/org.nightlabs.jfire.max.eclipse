...
package org.nightlabs.jfire.reporting.layout.action.print;
...
import org.nightlabs.jfire.reporting.config.ReportUseCase;
import org.nightlabs.jfire.reporting.config.ReportUseCaseRegistry;
import org.nightlabs.jfire.reporting.config.ReportViewPrintConfigModule;
import org.nightlabs.jfire.reporting.config.ReportViewPrintConfigModule.UseCaseConfig;
import org.nightlabs.jfire.reporting.layout.PreparedRenderedReportLayout;
import org.nightlabs.jfire.reporting.layout.RenderedReportLayoutProvider;
...
public class PrintReportLayoutUtil {
	...
	throws PrinterException 
	{
		RCPUtil.getSaveProgressMonitor(monitor).setTaskName(Messages.getString("org.nightlabs.jfire.reporting.layout.action.print.PrintReportLayoutUtil.saveProgressMonitor.lookupReportUseCaseTask.naem")); //$NON-NLS-1$
		ReportViewPrintConfigModule cfMod = ReportViewPrintConfigModule.sharedInstance();
...
//			Birt.OutputFormat format, 
	...
	throws PrinterException 
	{
		RCPUtil.getSaveProgressMonitor(monitor).setTaskName(Messages.getString("org.nightlabs.jfire.reporting.layout.action.print.PrintReportLayoutUtil.saveProgressMonitor.prepareLayoutTask.name")); //$NON-NLS-1$
		...
		RCPUtil.getSaveProgressMonitor(monitor).setTaskName(Messages.getString("org.nightlabs.jfire.reporting.layout.action.print.PrintReportLayoutUtil.saveProgressMonitor.lookupPrinterUseCaseTask.name")); //$NON-NLS-1$
		ReportViewPrintConfigModule cfMod = ReportViewPrintConfigModule.sharedInstance();
		UseCaseConfig useCaseConfig = cfMod.getReportUseCaseConfigs().get(reportUseCaseID);
...
//			throw new IllegalStateException("Could not lookup reportUseCase by the reportRegistryItemType: "+reportRegistryItemID.reportRegistryItemType);
	...
	throws PrinterException 
	{
		RCPUtil.getSaveProgressMonitor(monitor).setTaskName(Messages.getString("org.nightlabs.jfire.reporting.layout.action.print.PrintReportLayoutUtil.saveProgressMonitor.printTask.name")); //$NON-NLS-1$
