...
extends XComposite
{
	...
	public VoucherKeyDetailComposite(Composite parent, int style)
	{
		...
		new Label(this, SWT.NONE).setText(Messages.getString("org.nightlabs.jfire.voucher.detail.VoucherKeyDetailComposite.validityLabel.text")); //$NON-NLS-1$
		validity = new Text(this, SWT.BORDER | SWT.READ_ONLY);
		validity.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

		new Label(this, SWT.NONE).setText(Messages.getString("org.nightlabs.jfire.voucher.detail.VoucherKeyDetailComposite.nominalValueLabel.text")); //$NON-NLS-1$
		nominalValue = new Text(this, SWT.BORDER | SWT.READ_ONLY);
		nominalValue.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

		new Label(this, SWT.NONE).setText(Messages.getString("org.nightlabs.jfire.voucher.detail.VoucherKeyDetailComposite.restValueLabel.text")); //$NON-NLS-1$
	...
	{
		return Messages.getString("org.nightlabs.jfire.voucher.detail.VoucherKeyDetailComposite." + voucherKey.getValidityString()); //$NON-NLS-1$
	}
	...
	public void setVoucherKeyString(String _voucherKeyString)
	{
		...
		if (voucherKeyString == null)
			...
			validity.setText(Messages.getString("org.nightlabs.jfire.voucher.detail.VoucherKeyDetailComposite.validity_loadingData")); //$NON-NLS-1$
		...
		Job job = new Job(Messages.getString("org.nightlabs.jfire.voucher.detail.VoucherKeyDetailComposite.loadVoucherJob.name")) { //$NON-NLS-1$
			@Override
			protected IStatus run(ProgressMonitor monitor)
			{
				...
				Display.getDefault().asyncExec(new Runnable()
				{
					...
					{
						if (voucherKey == null) {
							validity.setText(Messages.getString("org.nightlabs.jfire.voucher.detail.VoucherKeyDetailComposite.validity_keyNotFound")); //$NON-NLS-1$
						}
						else {
