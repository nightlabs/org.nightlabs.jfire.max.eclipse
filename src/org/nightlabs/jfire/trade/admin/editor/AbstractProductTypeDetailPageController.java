...
implements IProductTypeDetailPageController
{
	...
	{
		monitor.beginTask(Messages.getString("org.nightlabs.jfire.trade.admin.editor.AbstractProductTypeDetailPageController.loadProductTypeDetailMonitor.task.name"), 2); //$NON-NLS-1$
		monitor.worked(1);
	...
	public void doSave(IProgressMonitor monitor) 
	{
		for (IFormPage page : getPages()) {
			if (page instanceof AbstractProductTypeDetailPage) {
				monitor.beginTask(Messages.getString("org.nightlabs.jfire.trade.admin.editor.AbstractProductTypeDetailPageController.storeProductTypeDetailMonitor.task.name"), 2); //$NON-NLS-1$
				monitor.worked(1);
