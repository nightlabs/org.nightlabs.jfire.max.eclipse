...
public class CompactedPersonRelationTreeNode extends PersonRelationTreeNode {
	...
	protected CompactedNodeStatus[] tuckedPathNodeStatuses = null;                // ::[Defn. 6.III]::
	...
	private CompactedNodeStatus nodeStatus = CompactedNodeStatus.UNSET;
	...
	protected void initCompactedTuckedNode(TuckedPathDosier tuckedPathDosier) {
		...
		if (this.tuckedPathDosier != null) {			
			...
			tuckedPathNodeStatuses = new CompactedNodeStatus[pathLen];
			
			for (int i=0; i<pathLen; i++) {
				tuckedPathNodesActualChildCount[i] = -1L;
				tuckedPathNodesTuckedChildCount[i] = -1L;
				tuckedPathNodeStatuses[i] = CompactedNodeStatus.UNSET;
			}
	 ...
	 */
	public void setNodeStatus(CompactedNodeStatus nodeStatus) {
	...
	// ------------------------------------------------------------------------------------- ++ ------------------------------->>
	/**
	 * @return the {@link CompactedNodeStatus} of this node. See ::[Defn. 6.III]::
	 */
	public CompactedNodeStatus getNodeStatus() {
		return nodeStatus;
	}
	...
	public boolean isNodeSet() {
		if (tuckedPathDosier == null) 
			return nodeStatus.equals(CompactedNodeStatus.NORMAL);
		
		for (CompactedNodeStatus nodeStatus : tuckedPathNodeStatuses)
			if (nodeStatus.equals(CompactedNodeStatus.UNSET))
				return false;
	...
	public long getChildNodeCount() {
		if (nodeStatus.equals(CompactedNodeStatus.UNSET) || nodeStatus.equals(CompactedNodeStatus.NORMAL))
			return super.getChildNodeCount();
		
		return getNodeStatus().equals(CompactedNodeStatus.COMPACTED) ? tuckedChildCount : actualChildCount;
	}
