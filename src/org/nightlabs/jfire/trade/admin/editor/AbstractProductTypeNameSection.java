...
implements IProductTypeSectionPart
{
	...
	public AbstractProductTypeNameSection(IFormPage page, Composite parent, int style) {
		this(page, parent, style, Messages.getString("org.nightlabs.jfire.trade.admin.editor.AbstractProductTypeNameSection.text")); //$NON-NLS-1$
	}
...
//		if (!getInheritanceSelection())
		if (getInheritanceSelection())
			...
			if (productType != null) {
				...
				Job job = new Job(Messages.getString("org.nightlabs.jfire.trade.admin.editor.AbstractProductTypeNameSection.loadExtendedProductTypeJob.name")) { //$NON-NLS-1$
					@Override
					protected IStatus run(ProgressMonitor monitor) {
						monitor.beginTask(Messages.getString("org.nightlabs.jfire.trade.admin.editor.AbstractProductTypeNameSection.loadExtendedProductTypeMonitor.task.name"), 10); //$NON-NLS-1$
