...
//		return new CompactedPersonRelationTreeNode();
		...
		for (PropertySetID personID : personIDs) {
			...
			if (treeNodeList != null && treeNodeList.size() == 1) {
				...
				for (int i=0; i<pathLen; i++) {
					...
					tuckedNodeInPath.setNodeStatus(i < pathLen-1 ? TuckedNodeStatus.TUCKED: TuckedNodeStatus.NORMAL);
	...
	protected Collection<ObjectID> retrieveChildObjectIDsByPropertySetIDs(CompactedPersonRelationTreeNode parentNode, ProgressMonitor monitor) {
		// Interceptor: If the parentNode is a COLLECTIVE node (i.e. it has a valid node-representative reference), then the child ObjectIDs that
		// need to be returned are not the parentNode's, but rather, the child ObjectIDs of the node-representative's.
		CompactedPersonRelationTreeNode nodeRepresentative = parentNode.getNodeRepresentative();
		if (nodeRepresentative != null) {
			if (logger.isDebugEnabled()) {
				logger.debug(" ### ##### FOUND a COLLECTIVE node while retrieveChildObjectIDsBy~~~PropertySetIDs() !!! +++++++++ <Poss. 1>");
			...
			// There are two independent cases here to consider:
			// [Case 1] We check to see if the node-representative carries a PersonRelationID. If so, we divert it to call the super class's method to handle
			//          the child-ObjectIDs' retrieval by PersonRelationID. 
			// [Case 2] If the node-representative carries a PropertySetID, then the parentNode (which incidentally is the COLLECTIVE shell-node) has only one 
			//          tucked-path node, which is this node-representative. In this case, we relinquish control back to the original method.
			
			// <-- See [Case 1]
			if (nodeRepresentative.getJdoObjectID() instanceof PersonRelationID)
	...
	protected Collection<ObjectID> retrieveChildObjectIDsByPersonRelationIDs(CompactedPersonRelationTreeNode parentNode, ProgressMonitor monitor) {
		...
		CompactedPersonRelationTreeNode nodeRepresentative = parentNode.getNodeRepresentative();
		if (nodeRepresentative != null) {
			logger.debug(" ### ##### FOUND a COLLECTIVE node while retrieveChildObjectIDsBy~~~PersonRelationIDs() !!! +++++++++ <Poss. 2>");
			logger.debug(" ### ##### Node identified as COLLECTIVE: meNodeID = " + parentNode.meNodeID + ", nodeRepresentative.meNodeID = " + nodeRepresentative.meNodeID);

			// With respect to the two independent cases above, it is impossible that the node-representative 
			// at this point here carries a PropertySetID.
			return super.retrieveChildObjectIDsByPersonRelationIDs(nodeRepresentative, monitor); 
		}
