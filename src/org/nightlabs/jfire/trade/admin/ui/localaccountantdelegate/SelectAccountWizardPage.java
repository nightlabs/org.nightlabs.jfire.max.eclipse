package org.nightlabs.jfire.trade.admin.ui.localaccountantdelegate;
...
extends WizardHopPage
{
	...
	public SelectAccountWizardPage(Currency currency, Account preselectedAccount, AccountTypeID selectedAccountTypeID)
	{
		super(SelectAccountWizardPage.class.getName(), "Create account"); //$NON-NLS-1$
	...
	public Control createPageContents(Composite parent)
	{
		...
		selectAccount.setText("Select the following account:");
		...
		dummy.getName().setText(NLLocale.getDefault().getLanguage(), "Loading data..."); //$NON-NLS-1$
		accountList.addElement(dummy);

		Job job = new Job("Loading accounts") { 
			@Override
...
//					accountSearchFilter.setAnchorTypeID(VoucherLocalAccountantDelegate.ACCOUNT_ANCHOR_TYPE_ID_VOUCHER);
					accountSearchFilter.setAccountTypeID(selectedAccountTypeID);
